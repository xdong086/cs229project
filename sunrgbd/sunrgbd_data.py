...
def data_viz(data_dir, dump_dir=os.path.join(BASE_DIR, 'data_viz_dump')):  
    ...
    for idx in range(len(sunrgbd)):
        dump_dir_idx = os.path.join(dump_dir, str(idx))
        #data_idx = idxs[idx]
        data_idx = (idx + 1) 
        ...
        if not os.path.exists(dump_dir_idx):
            os.mkdir(dump_dir_idx)
        Image.fromarray(img).save(os.path.join(dump_dir_idx,'img_depth.jpg'))
        ...
        for i,obj in enumerate(objects):
            ...
            cv2.putText(img, '%d %s'%(i,obj.classname), (max(int(obj.xmin),15),
                ...
                (255,0,0), 2)
        Image.fromarray(img).save(os.path.join(dump_dir_idx, 'img_box2d.jpg'))

        # Dump point cloud
        np.save(os.path.join(dump_dir_idx, 'pc.npy'), pc)
        ...
        for num_point in [10000,20000,40000,80000]:
            ...
            pc_util.write_ply_rgb(sampled_pcrgb[:,0:3],
                ...
                os.path.join(dump_dir_idx, 'pcrgb_%dk.obj'%(num_point//1000)))
        ...
        if len(oriented_boxes)>0:
            ...
            pc_util.write_oriented_bbox(oriented_boxes,
                os.path.join(dump_dir_idx, 'obbs.ply'))
        else:
            print('-'*30)
            continue
        ...
        pc_util.write_ply(pc_box3d, os.path.join(dump_dir_idx, 'box3d_corners.ply'))
        pc_util.write_ply(pc_ori3d, os.path.join(dump_dir_idx, 'box3d_ori.ply'))
        print('-'*30)
        print('Point clouds and bounding boxes saved to PLY files under %s'%(dump_dir_idx))
...
def extract_sunrgbd_data_box2d(idx_filename, split, output_folder,
    type_whitelist=DEFAULT_TYPE_WHITELIST, use_v1=False, skip_empty_scene=True, use_float=True):
    """ Extract 2d bounding boxes in image coordinates
    ...
    Args:
        ...
        use_v1: use the SUN RGB-D V1 data
        skip_empty_scene: if True, skip scenes that contain no object (no objet in whitelist)
        use_float: if True, use float number to store 2d bbox, otherwise convert to integer
            (we found the 2d bbox coordinates are not always integer in sunrgbd_trainval/lavel_v1)

    Dumps:
        <id>_2dbbox.npy of K x ([class_prob],[box2d]) where K is the number of objects, 
        class_prob is the box2d class prediction probability, box2d is [xmin,ymin,xmax,ymax]
    """
    dataset = sunrgbd_object('./sunrgbd_trainval', split, use_v1=use_v1)
    data_idx_list = [int(line.rstrip()) for line in open(idx_filename)]
    classlen = len(type_whitelist)
    ...
    for data_idx in data_idx_list:
        ...
        for obj in objects:
            ...
            prob = np.zeros(classlen, dtype='float')
            prob[int(sunrgbd_utils.type2class[obj.classname])] = 1.0
            prob_list.append(prob)
            # 2d bounding box
            if use_float:
                imgbbox = obj.box2d.astype(float)
            else:
                imgbbox = obj.box2d.astype(int)
            bbox2d_list.append(imgbbox)

        if len(bbox2d_list) == 0:
            bbox2d = np.zeros((0,4))
        else:
            bbox2d =  np.vstack(bbox2d_list)

        if len(prob_list) == 0:
            bbox2d_prob = np.zeros((0, classlen))
        else:
            bbox2d_prob = np.vstack(prob_list)
        ...
        np.save(os.path.join(output_folder, '%06d_bbox2d.npy'%(data_idx)), bbox2d)
        np.save(os.path.join(output_folder, '%06d_bbox2d_prob.npy'%(data_idx)), bbox2d_prob)
        np.save(os.path.join(output_folder, '%06d_calib_Rtilt.npy'%(data_idx)), Rtilt)
        np.save(os.path.join(output_folder, '%06d_calib_K.npy'%(data_idx)), K)

def sunrgbd_to_coco(idx_filename, split, output_filename="coco.json",
    type_whitelist=DEFAULT_TYPE_WHITELIST, use_v1=False, skip_empty_scene=True):
    """
    Converting SUN RGB-D dataset into COCO dataset format
    """
    info = {
        "description": "SUN RGB-D",
        "version": "1.0"
    }
    licenses = {
        "id": 1,
    ...
    categories = []
    for idx in range(len(type_whitelist)):
        categories.append({
            "supercategory": "object",
            "id": idx+1,
            "name": type_whitelist[idx]
        })
    ...
    for data_idx in data_idx_list:
        print(data_idx)
        im = Image.open(os.path.join(BASE_DIR, 'sunrgbd_trainval', 'image', f'{data_idx:06d}.jpg'))
        image = {"license": 1, 
        "id": data_idx,
        "height": im.size[1],
        "width": im.size[0],
        "file_name": f"{data_idx:06d}.jpg"}
        im.close()
        images.append(image)
        ...
        for obj in objects:
            ...
            bbox2dwidth = max(0.0, xmax - xmin)
            bbox2dheight = max(0.0, ymax - ymin)

            annotation = {"segmentation": [], "iscrowd": 0,
            "bbox": [xmin, ymin, bbox2dwidth, bbox2dheight],
            "id": annotation_id_count,
            "image_id": data_idx,
            "category_id": type_whitelist.index(obj.classname)+1,
            "area": bbox2dwidth*bbox2dheight}
            annotation_id_count += 1

            annotations.append(annotation)


    json_obj = {"info": info, "licenses": licenses, "images": images, 
...
def convert_coco_to_npy(input_json, output_folder, 
    type_whitelist=DEFAULT_TYPE_WHITELIST, score_threshold=0.4):
    """
    Convert output from dectron2 coco json to npy
    """
    classlen = len(type_whitelist)
    ...
    with open(input_json) as json_file:
        ...
        for det in json_obj:
            ...
            if previd != image_id:
                # save bbox and bbox_prod
                if len(bbox2d_list) == 0:
                    bbox2d = np.zeros((0,4))
                else:
                    bbox2d = np.vstack(bbox2d_list)

                if len(bbox2d_prob_list) == 0:
                    bbox2d_prob = np.zeros((0, classlen))
                else:
                    bbox2d_prob = np.vstack(bbox2d_prob_list)

                np.save(os.path.join(output_folder, '%06d_bbox2d.npy'%(previd)), bbox2d)
                np.save(os.path.join(output_folder, '%06d_bbox2d_prob.npy'%(previd)), bbox2d_prob)
                previd = image_id
                bbox2d_list = []
                bbox2d_prob_list = []
            ...
            imgbox = [xmin, ymin, xmin + width, ymin + height]
            bbox2d_list.append(imgbox)
            #bbox2d_list.append(bbox)
            prob = np.zeros(classlen, dtype='float')
            prob[category_id - 1] = 1
            bbox2d_prob_list.append(prob)
...
if __name__=='__main__':
    ...
    parser.add_argument('--gen_v1_2dbbox', action='store_true', help='Generate V1 2d bounding box.')
    parser.add_argument('--gen_v2_2dbbox', action='store_true', help='Generate V2 2d bounding box.')
    parser.add_argument('--gen_coco', action='store_true', help='Generate coco format json.')
    parser.add_argument('--from_2d', action='store_true', help='Create evaluation dataset from 2d detector json.')
    ...
    if args.gen_v1_2dbbox:
        extract_sunrgbd_data_box2d(os.path.join(BASE_DIR, 'sunrgbd_trainval/train_data_idx.txt'),
            split = 'training',
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_pc_bbox_votes_50k_v1_train'),
            use_v1=True, skip_empty_scene=False)
        extract_sunrgbd_data_box2d(os.path.join(BASE_DIR, 'sunrgbd_trainval/val_data_idx.txt'),
            split = 'training',
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_pc_bbox_votes_50k_v1_val'),
            use_v1=True, skip_empty_scene=False)

    if args.gen_v2_2dbbox:
        extract_sunrgbd_data_box2d(os.path.join(BASE_DIR, 'sunrgbd_trainval/train_data_idx.txt'),
            split = 'training',
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_pc_bbox_votes_50k_v2_train'),
    ...
    if args.gen_coco:
        sunrgbd_to_coco(os.path.join(BASE_DIR, 'sunrgbd_trainval/train_data_idx.txt'),
            split = 'training',
            output_filename = os.path.join(BASE_DIR, 'sunrgbd_trainval/coco/sunrgbd_train2017.json'),
            use_v1=True, skip_empty_scene=False)
        sunrgbd_to_coco(os.path.join(BASE_DIR, 'sunrgbd_trainval/val_data_idx.txt'),
            split = 'training',
            output_filename = os.path.join(BASE_DIR, 'sunrgbd_trainval/coco/sunrgbd_val2017.json'),
            use_v1=True, skip_empty_scene=False)

    if args.from_2d:
        convert_coco_to_npy(input_json=os.path.join(BASE_DIR, 'coco_instances_results.json'),
            output_folder = os.path.join(BASE_DIR, 'sunrgbd_pc_bbox_votes_50k_v1_val'))