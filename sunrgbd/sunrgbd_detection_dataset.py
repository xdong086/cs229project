...
class SunrgbdDetectionVotesDataset(Dataset):
    def __init__(self, split_set='train', num_points=20000,
        ...
        augment=False, scan_idx_list=None, use_box2d=False):
    ...
    def __getitem__(self, idx):
        ...
        bbox2ds = np.load(os.path.join(self.data_path, scan_name)+'_bbox2d.npy')
        bbox2d_probs = np.load(os.path.join(self.data_path, scan_name)+'_bbox2d_prob.npy')
        calib_Rtilt = np.load(os.path.join(self.data_path, scan_name)+'_calib_Rtilt.npy')
        calib_K = np.load(os.path.join(self.data_path, scan_name)+'_calib_K.npy')

        if self.use_color and self.use_box2d:
            raise NotImplemented('color and 2d bounding box at the same time is not implemented') 
        if not self.use_color:
            #point_cloud = point_cloud[:,0:3]
            point_cloud = get_box2d_feature(point_cloud, bbox2ds, bbox2d_probs, calib_Rtilt, calib_K)
        else:
...
def get_box2d_feature(point_cloud, bbox2d, bbox2d_prob, calib_Rtilt, calib_K):
    """
    Return point_cloud with 2d bounding box feature [x,y,z, [feture]]
    """
    #N, M = point_cloud.shape
    boxnum = bbox2d.shape[0]
    point_cloud = point_cloud[:,0:3]
    problen = bbox2d_prob.shape[1]
    point_cloudlen = point_cloud.shape[0]

    point_cloud_prob = np.zeros([point_cloudlen, problen])
    uv = project_point_clound_to_image(point_cloud, calib_Rtilt, calib_K)
    for idx in range(boxnum):
        box = bbox2d[idx]
        p1 = box[0:2]
        p2 = box[2:4]
        mask = np.where(np.all((uv >= p1) & (uv <= p2), axis=1))[0]
        point_cloud_prob[mask] = bbox2d_prob[idx]

    return np.concatenate((point_cloud, point_cloud_prob), axis=1)
