...
def dump_results(end_points, dump_dir, config, inference_switch=False):
    ...
    for i in range(batch_size):
        ...
        np.save(os.path.join(dump_dir, '%06d_pc.npy'%(idx_beg+i)), np.vstack(pc))
        ...
        if np.sum(objectness_prob>DUMP_CONF_THRESH)>0:
            ...
            for j in range(num_proposal):
                ...
                obbs.append(obb)
            print("aaaaaaaaaaaaaaaaa", len(obbs))
            if len(obbs)>0:
                obbs = np.vstack(tuple(obbs)) # (num_proposal, 7)
                np.save(os.path.join(dump_dir, '%06d_pred_bbox_obbs.npy'%(idx_beg+i)), obbs[np.logical_and(objectness_prob>DUMP_CONF_THRESH, pred_mask[i,:]==1),:])
    ...
    for i in range(batch_size):
        ...
        for j in range(gt_center.shape[1]):
            ...
            obbs.append(obb)
        print("bbbbbbbbbbb", len(obbs))
        if len(obbs)>0:
            obbs = np.vstack(tuple(obbs)) # (num_gt_objects, 7)
            np.save(os.path.join(dump_dir, '%06d_gt_bbox_obbs.npy'%(idx_beg+i)), obbs)
